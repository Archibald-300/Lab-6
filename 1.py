...
def new_rectangle():
    ...
    for i in range(number_of_rectangles):
        ...
        if ((abs(time - var)<= 10) or (time >= 100)) and (x_rectangle[i] < 690 - Length_x_rectangle[i]) and (x_rectangle[i] > Length_x_rectangle[i] + 15):
            time = 0
            Vx_rectangle[i] = - Vx_rectangle[i]
        
        #conditions of change of the rectangles' direction becase of borders    
        if (x_rectangle[i] >= length_screen - 50 - Length_x_rectangle[i]) or (x_rectangle[i] <= Length_x_rectangle[i] + 9):
            Vx_rectangle[i] = -Vx_rectangle[i]
        if (y_rectangle[i] >= high_screen - 50 - length_y_rectangle[i]) or (y_rectangle[i] <= length_y_rectangle[i] + 9):
...
screen.fill(white)
text3 = font.render('Please, write your nickname,', True, green, BLUE) 
text3Rect = text3.get_rect() 
text3Rect.center = (400, 200) 
screen.blit(text3, text3Rect)
text3 = font.render('push Enter-button afterwards', True, green, BLUE) 
...
while done == False:
    for event in pygame.event.get():
            ...
            pygame.display.update()
#creating text1 with number of available tries
text1 = quit_button.render("Available tries:" + str(number_of_available_clicks), True, green, BLUE) 
text1Rect = text1.get_rect() 
text1Rect.center = (610, 16) 
#counting of score
while done:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            for i in range(number_of_rectangles):
                if (event.pos[0] - x_rectangle[i] <= Length_x_rectangle[i]) and (event.pos[0] - x_rectangle[i] >= 0) and (event.pos[1] - y_rectangle[i] <= length_y_rectangle[i]) and (event.pos[1] - y_rectangle[i] >= 0):
...
screen.fill(white)
#Quit button
quit_text = quit_button.render(quit, True, green, BLUE) 
